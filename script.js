class Game {
  constructor() {
    ...
    // 3 colors used in the game
    this.colors = ["#FF4571", "#FFD145", "#8260F6"];
    this.colorsRGBA = ["rgba(255, 69, 113, 1)", "rgba(255, 209, 69, 1)", "rgba(130, 96, 246, 1)"];

    // intial color of the ball
    this.color = this.colors[0]; 

    // holder to prevent ball colors from repeating
    this.prevColor = null;
  }

  // CalculateScale Function() is called in controller and anywhere needed
  // to recalculate scale on screen, resize or device rotation
  calculateScale() {
    // screen width
    ...
    // no. of steps (stick width + margin) it takes from one end to another
    this.steps = this.screen / this.stickWidth;
  }

  // Creating as many sticks we need to fill the screen
  // from start to end of the screen
  generateSticks() {
  ...
  */
  ...
  start() {
    ...
    // Cleans up the stick and ball holders
    // and generates new screen
    $('#sticks, .scene .ball-holder').html('');
    $('#score').text(this.score);
    this.generateSticks();
    this.generateBall();

    // disabling scene animations for Phones
    ...
    // resets timescale to normal as game speeds up
  ...
  scaleScreen() {
    TweenMax.killAll(); // prevents multiple calls on resize
  ...
  // If game is running it stops and shows score
  // If game has stops takes player to main menu
  scaleScreenAndRun() {
    this.scaleScreen();
  ...
  // removes the first stick and adds one too the end
  ...
  // Ball bouncing animation CodeBlock
  // Checks if ball and stick colors match
...
class Color {
  ...
  // As we as adds verbal name of color
  changeColor(el) {
  ...
  // Adds the effect specific particles to stick
  setEffect(el) {
...
class Animation {
  // Creates and positions the small glow elements on screen
  ...
  // Creates animations for sticks
  // effects is chosen by random
  // And one of the three functions is used
  playBubble(el) {
  ...
  static sceneAnimation() {
    ...
    // animates the small glows in circular motion
